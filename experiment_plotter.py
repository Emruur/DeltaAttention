...
def plot_1d_column(df, param_name, output_path):
    summary = df.groupby(param_name)[['Accuracy', 'Attn_Sparsity']].mean().reset_index()
    summary = summary.sort_values(param_name)

    n_rows = len(summary)
    norm_acc = mcolors.Normalize(vmin=summary['Accuracy'].min(), vmax=summary['Accuracy'].max())
    norm_spar = mcolors.Normalize(vmin=summary['Attn_Sparsity'].min(), vmax=summary['Attn_Sparsity'].max())
    ...
    cb_spar.set_label('Attn Sparsity (Top-Right)', color='purple', fontsize=9, fontweight='bold')
...
if __name__ == "__main__":
    ...
    if len(params) == 1:
        ...
        # Note: Heatmap logic omitted for brevity, identical to your original code
        print(f"[Logic] 2 Parameters detected. Ensure plot_heatmap is defined if using default mode.")
