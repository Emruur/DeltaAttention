...
def load_and_detect_data(experiment_dir):
    ...
    # FIX: If no parameters vary (single config run), use Config_ID so plotting logic doesn't crash
    if len(varying_params) == 0:
        print("[System] No varying parameters detected. Using 'Config_ID' to plot single configuration.")
        varying_params = ["Config_ID"]
...
def plot_1d_column(df, param_name, output_path, sparsity_col='Attn_Sparsity', sparsity_label='Attn Sparsity'):
    ...
    # Safely handle the case where max == min (single config) to avoid divide-by-zero visual bugs
    acc_min, acc_max = summary['Accuracy'].min(), summary['Accuracy'].max()
    spar_min, spar_max = summary[sparsity_col].min(), summary[sparsity_col].max()
    if acc_min == acc_max: acc_min, acc_max = acc_min - 0.1, acc_max + 0.1
    if spar_min == spar_max: spar_min, spar_max = spar_min - 0.1, spar_max + 0.1

    norm_acc = mcolors.Normalize(vmin=acc_min, vmax=acc_max)
    norm_spar = mcolors.Normalize(vmin=spar_min, vmax=spar_max)
...
def process_type2(df, varying_param, output_dir):
    ...
    """
    print(f"[Type 2] Processing data for parameter: {varying_param}")
    ...
    # 2. Calculate Average Accuracy across all tasks
...
if __name__ == "__main__":
    ...
    if df.empty:
        print("[Error] No valid task data found. Exiting.")
        exit()
    ...
    if args.plot_type == 'type2':
        if len(params) > 1:
            print(f"[Warning] Type 2 expects exactly 1 varying parameter. Found {len(params)}: {params}")
    ...
    if len(params) == 1:
        param_name = params[0]
        print(f"[Logic] 1 Parameter detected (or forced): {param_name}")
