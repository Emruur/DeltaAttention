...
\begin{enumerate}
    ...
    \item \textbf{post-RoPE key-shuffled}: pre-RoPE key vectors shuffled, RoPE then applied
\end{enumerate}
...
After RoPE is on the shuffled keys, the similarity recovers to nearly the level of configuration 3.
...
RoPE recovers adjacent similarity for shuffled sequences, contributing a positional structure on top of the content signal.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{../assets/query_similarity_mean_by_layer.png}
    \caption{Mean adjacent query cosine similarity per layer (averaged over all heads) for LLaMA-3 8B Instruct at layers 0, 8, 16, and 23, under four configurations: pre-RoPE real, pre-RoPE shuffled, post-RoPE real, and post-RoPE shuffled.}
