...
We evaluate delta-based tile scoring (DTS) against MInference, FlexPrefill, and XAttention. We exclude SpargeAttention from our comparisons because its implementation is bundled with quantization. All baselines are run directly from their original GitHub repositories without reimplementation. We report DTS at two cosine thresholds, 0.72 and 0.78, both at a recall of $r{=}0.90$. We evaluate these methods on RULER (100 samples, 13 tasks) and LongBench (16 tasks), alongside an isolated speedup analysis from 4K to 512K tokens.
...
\begin{table}[!ht]
\centering
\caption{RULER accuracy (\%) comparing delta-based tile scoring against baseline and block-sparse methods on LLaMA-3.1 8B Instruct (100 samples, 13 tasks). DTS and FlexPrefill are both at recall/coverage target $0.90$ ($r{=}0.90$ and $\gamma{=}0.90$ respectively). Best score per column in \textbf{bold}.}
...
\begin{table}[!ht]
\centering
\caption{End-to-end prefill speedup (128K tokens) and accuracy (\%) on RULER and LongBench for DTS cosine thresholds on LLaMA-3.1 8B Instruct ($r{=}0.90$). Attention density is the fraction of computed sparse attention. Score\% represents the scoring overhead. Score ms and Attn ms indicate the respective phase latencies.}
...
The results above use LLaMA-3.1 8B Instruct throughout. To check whether DTS's behavior is specific to that model, we repeat the full RULER, LongBench, and speedup evaluation on Qwen2.5-7B-Instruct-1M, using the same two operating points (cos\,0.72 and cos\,0.78 at $r{=}0.90$) and the same baselines.
...
\begin{table}[!ht]
\centering
\caption{RULER accuracy (\%) comparing delta-based tile scoring against baseline and block-sparse methods on Qwen2.5-7B-Instruct-1M (100 samples, 13 tasks). DTS and FlexPrefill are both at recall/coverage target $0.90$ ($r{=}0.90$ and $\gamma{=}0.90$ respectively). Best score per column in \textbf{bold}.}
...
\midrule
DTS cos\,0.72 & 94.01 & 92.38 & 92.56 & 87.45 & 83.69 & 78.60 & 88.12 \\
DTS cos\,0.78 & 94.02 & 93.54 & 92.59 & 89.38 & 84.54 & 79.26 & 88.89 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\centering
\small
\caption{LongBench scores on Qwen2.5-7B-Instruct-1M (100 samples per task, 16 tasks). DTS and FlexPrefill are both at recall/coverage target $0.90$ ($r{=}0.90$ and $\gamma{=}0.90$ respectively). Task abbreviations as in Table~\ref{tab:longbench}. Best score per column in \textbf{bold}. $\dagger$MInference failed on 2 of 16 tasks (QMSum, RepoBench-P); its reported average is over the 14 completed tasks and is not directly comparable to the other rows.}
...
\midrule
DTS cos\,0.72 & 29.89 & \textbf{46.48} & 52.04 & \textbf{58.56} & 53.31 & 29.22 & 35.12 & 24.19 & 25.86 & 72.00 & 83.17 & \textbf{46.51} & 4.00 & 98.00 & 45.57 & 35.15 & 46.19 \\
DTS cos\,0.78 & 28.16 & 42.79 & 50.85 & 55.59 & 52.46 & 29.88 & 35.11 & 24.33 & 25.57 & \textbf{74.00} & \textbf{83.84} & 46.18 & 10.00 & 97.00 & 46.52 & \textbf{36.90} & 46.20 \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\end{axis}
\end{tikzpicture}
\caption{End-to-end prefill speedup relative to SDPA across sequence lengths on Qwen2.5-7B-Instruct-1M (4$\times$H100, layer-sharded). XAttention and DTS are reported to 256K; both OOM at 512K on this hardware.}
...
XAttn $s{=}16$ ($r{=}0.90$)       & 72.0 & 59.9 & 51.7 & 61.2 & 0.95 & 1.22 & 1.18 \\
\midrule
DTS $\delta{=}15$ ($r{=}0.90$)   & 71.1 & 58.6 & 52.6 & 60.8 & 0.98 & 1.21 & 1.33 \\
\bottomrule
\end{tabular}%
}
\end{table}
