...
\begin{tikzpicture}[>=stealth, thick]
  ...
  \def\cell{0.15} % cell size for left and right
  \def\bcell{1.2} % big cell size = 8 * 0.15
  \def\mcell{0.4} % small (3x3) grid cell size
  ...
  \begin{scope}[shift={(0,0)}]
    ...
    \foreach \br in {0,1,2} {
      \foreach \r in {0,...,7} {
        ...
        \ifnum\isrow=1 \colorlet{fcol}{dark} \else \colorlet{fcol}{white} \fi
        \fill[fcol] (-0.1-\cell, {-\br*\bcell - \r*\cell}) rectangle (-0.1, {-\br*\bcell - \r*\cell - \cell});
        \draw[gridcolor, very thin] (-0.1-\cell, {-\br*\bcell - \r*\cell}) rectangle (-0.1, {-\br*\bcell - \r*\cell - \cell});
      }
    }
    \draw[very thick] (-0.1-\cell, 0) rectangle (-0.1, -3*\bcell);
    \foreach \br in {1,2} {
      \draw[very thick] (-0.1-\cell, -\br*\bcell) -- (-0.1, -\br*\bcell);
    }
    \node[left, font=\small\bfseries] at (-0.1-\cell, -1.5*\bcell) {$\hat{Q}$};
    ...
    \foreach \bc in {0,1,2} {
      \foreach \c in {0,...,7} {
        ...
        \ifnum\iscol=1 \colorlet{fcol}{dark} \else \colorlet{fcol}{white} \fi
        \fill[fcol] ({\bc*\bcell + \c*\cell}, 0.1) rectangle ({\bc*\bcell + \c*\cell + \cell}, 0.1+\cell);
        \draw[gridcolor, very thin] ({\bc*\bcell + \c*\cell}, 0.1) rectangle ({\bc*\bcell + \c*\cell + \cell}, 0.1+\cell);
      }
    }
    \draw[very thick] (0, 0.1) rectangle (3*\bcell, 0.1+\cell);
    \foreach \bc in {1,2} {
      \draw[very thick] (\bc*\bcell, 0.1) -- (\bc*\bcell, 0.1+\cell);
    }
    \node[above, font=\small\bfseries] at (1.5*\bcell, 0.1+\cell) {$\hat{K}$};
  \end{scope}

  % Arrow 1
  \draw[->, very thick] (3*\bcell + 0.2, -0.5*\bcell) -- ++(0.6, 0);

  % PART 2: Tile Scores (weight per tile, normalized per query row)
  \begin{scope}[shift={(3*\bcell + 1.0, -0.5*\bcell + 0.5*\mcell)}]
    ...
    \node[above, align=center, font=\small\bfseries] at (1.5*\mcell, 0) {Tile Scores};
  \end{scope}

  % Arrow 2
  \draw[->, very thick] (3*\bcell + 1.0 + 1.5*\mcell, -0.5*\bcell - 3*\mcell) -- ++(0, -0.4);

  % PART 3: Block Selection
  \begin{scope}[shift={(3*\bcell + 1.0, -2.5*\bcell + 1.5*\mcell)}]
    \foreach \br in {0,1,2} {
      \foreach \bc in {0,1,2} {
        \pgfmathtruncatemacro{\issel}{(\br==\bc) || (\br+\bc==2)}
        \ifnum\issel=1
    ...
    \draw[very thick] (0,0) rectangle (3*\mcell, -3*\mcell);
    \node[below, align=center, font=\small\bfseries] at (1.5*\mcell, -3*\mcell - 0.2) {Block\\Selection};
  \end{scope}

  % Arrow 3
  \draw[->, very thick] (3*\bcell + 1.0 + 3*\mcell + 0.2, -2.5*\bcell) -- ++(0.6, 0);

  % PART 4: Block Sparse Attention
  \begin{scope}[shift={(3*\bcell + 1.0 + 3*\mcell + 1.0, 0)}]
    \fill[bglight] (0,0) rectangle (3*\bcell, -3*\bcell);
    \draw[gridcolor, very thin, step=\cell] (0,0) grid (3*\bcell, -3*\bcell);
