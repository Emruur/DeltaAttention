...
def get_sims(input_ids):
    """Single forward pass → returns pre and post RoPE key adjacency sims."""
    pre_raw = {}   # stores raw [h, seq, d] tensors for optional shuffling
    hooks   = []
    ...
    for layer_idx in LAYERS:
        k_pre = pre_raw[layer_idx]                                    # [h, seq, d]
        k_post = extract_layer_keys(outputs.past_key_values, layer_idx)[0]  # [h, seq, d]
        k_post = k_post.float()

        perm = torch.randperm(k_pre.shape[1])

        pre_real[layer_idx]  = adj_cos(k_pre)
        pre_shuf[layer_idx]  = adj_cos(k_pre[:, perm, :])
        post_real[layer_idx] = adj_cos(k_post)
        post_shuf[layer_idx] = adj_cos(k_post[:, perm, :])
...
STYLES = [
    ...
    (pre_shuf,  "pre-RoPE key-shuffled",  "steelblue", "--", 1.0),
    (post_real, "post-RoPE real",         "tomato",    "-",  1.4),
    (post_shuf, "post-RoPE key-shuffled", "tomato",    "--", 1.0),
]
...
fig.suptitle(
    "Adjacent key cosine similarity · pre/post-RoPE × real/key-shuffled\n"
