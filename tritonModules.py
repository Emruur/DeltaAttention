...
@triton.jit
def parallel_scatter_pack_kernel(
    in_ptr, packed_ptr, keep_mask_ptr, index_map_ptr,
    stride_in_b, stride_in_h, stride_in_s, stride_in_d,
    stride_p_b, stride_p_h, stride_p_a, stride_p_d,
    stride_km_b, stride_km_h, stride_km_s,
    stride_im_b, stride_im_h, stride_im_s,
    seq_len, head_dim, num_heads,
    BLOCK_S: tl.constexpr, BLOCK_D: tl.constexpr
    ...
    in_ptrs = in_ptr + batch_idx * stride_in_b + head_idx * stride_in_h + offs_s[:, None] * stride_in_s + offs_d[None, :] * stride_in_d
    im_ptrs = index_map_ptr + batch_idx * stride_im_b + head_idx * stride_im_h + offs_s * stride_im_s
    
    # Load inputs and target destinations
    keys = tl.load(in_ptrs, mask=mask_s[:, None] & mask_d[None, :], other=0.0)
    dest_indices = tl.load(im_ptrs, mask=mask_s, other=0)
    
    # Scatter write ONLY the kept tokens
    packed_ptrs = packed_ptr + batch_idx * stride_p_b + head_idx * stride_p_h + dest_indices[:, None] * stride_p_a + offs_d[None, :] * stride_p_d
    tl.store(packed_ptrs, keys, mask=(keep_mask[:, None] == 1) & mask_s[:, None] & mask_d[None, :])

@triton.jit
def smart_absolute_pack_kernel(
    in_ptr, packed_ptr, index_map_ptr, counts_ptr,         # Pointers
    threshold_sq,                                          # Pre-squared threshold
    stride_in_b, stride_in_h, stride_in_s, stride_in_d,    # Input strides
    stride_p_b, stride_p_h, stride_p_a, stride_p_d,        # Packed strides
    stride_im_b, stride_im_h, stride_im_s,                 # Index Map strides
    ...
    packed_seq_ptr = packed_ptr + pid_b * stride_p_b + pid_h * stride_p_h
    im_seq_ptr = index_map_ptr + pid_b * stride_im_b + pid_h * stride_im_h
    
    offs_d = tl.arange(0, BLOCK_D)
    mask_d = offs_d < head_dim
    
    # --- PROCESS TOKEN 0 (Anchor) ---
    ptrs_0 = in_seq_ptr + 0 * stride_in_s + offs_d * stride_in_d
    ref_state = tl.load(ptrs_0, mask=mask_d, other=0.0)
    
    packed_ptrs_0 = packed_seq_ptr + 0 * stride_p_a + offs_d * stride_p_d
    tl.store(packed_ptrs_0, ref_state, mask=mask_d)
    
    active_idx = 0
    tl.store(im_seq_ptr + 0 * stride_im_s, active_idx) 
    active_idx += 1 
    ...
    for i in range(1, seq_len):
        ...
        if sq_dist > threshold_sq:
            # STORE ABSOLUTE STATE, NOT DIFF
            curr_packed_ptrs = packed_seq_ptr + active_idx * stride_p_a + offs_d * stride_p_d
            tl.store(curr_packed_ptrs, curr_state, mask=mask_d)
            ref_state = curr_state
            
            tl.store(im_seq_ptr + i * stride_im_s, active_idx)
            active_idx += 1
        else:
            # Route dropped token to previous active slot
            tl.store(im_seq_ptr + i * stride_im_s, active_idx - 1)
            
    # Save exact count of active tokens for dynamic slicing
